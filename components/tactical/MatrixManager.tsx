  ...
  return (
    <div className="max-w-4xl mx-auto h-full flex flex-col">
      <div className="flex items-end justify-between border-b border-void-border pb-6 mb-6">
        <div>
          <h2 className="text-3xl font-bold text-white tracking-widest uppercase mb-2 flex items-center gap-3"><Hexagon className="w-8 h-8 text-tech-green" />Gestor de Matrices</h2>
      ...
      <div className="border border-void-border bg-void-gray/10 flex-1 overflow-hidden flex flex-col relative">
        ...
        <div className="overflow-y-auto flex-1 custom-scrollbar">
          {loading ? (<div className="p-8 text-center text-tech-green animate-pulse font-mono">LOADING ONTOLOGY...</div>) : (
             matrices.map((matrix) => (
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
             <MotionDiv initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setIsModalOpen(false)} />
             <MotionDiv initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-void-black border border-void-border w-full max-w-lg shadow-[0_0_50px_rgba(0,0,0,0.8)] p-8">
                <div className="flex justify-between items-start mb-6"><h3 className="text-xl font-bold text-white uppercase tracking-wider flex items-center gap-2"><Server className="w-5 h-5 text-tech-green" />Crear Nueva Entidad</h3><button onClick={() => setIsModalOpen(false)} className="text-gray-500 hover:text-white"><X className="w-5 h-5" /></button></div>
                <div className="space-y-6">
                   <TechInput label="CÓDIGO DE MATRIZ (ID ÚNICO)" placeholder="EJ: ANUEL" value={formData.code} onChange={(e) => setFormData({...formData, code: e.target.value.toUpperCase().slice(0, 10)})} maxLength={10} />
                   <TechInput label="NOMBRE VISUAL (VISUALIZACIÓN)" placeholder="EJ: Anuel AA" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} />
                   <div className="space-y-1">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                         {['PRIMARY', 'SECONDARY'].map((type) => (
                            <button key={type} onClick={() => setFormData({...formData, type: type as any})} className={cn("py-2 border text-xs font-mono transition-all duration-200", formData.type === type ? "border-tech-green bg-tech-green/20 text-white shadow-[0_0_10px_rgba(0,255,65,0.2)]" : "border-void-border text-gray-600 hover:bg-void-gray")}>{type}</button>
                         ))}
                      </div>
                   </div>
                   <div className="pt-4 flex justify-end gap-4"><TechButton variant="ghost" label="CANCELAR" onClick={() => setIsModalOpen(false)} /><TechButton variant="primary" label={saving ? "INICIALIZANDO..." : "CONFIRMAR CREACIÓN"} onClick={handleSubmit} disabled={saving}/></div>
                </div>
             </MotionDiv>
          </div>
        )}
